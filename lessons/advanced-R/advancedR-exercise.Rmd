...
**Exercise**: Open the data in excel or google sheets. Make a list of things you would need to do to original data set to get into the tidy format. 
...
```
Missing data in datasets is common and statistical models are able to hande NA data, so they should be left as is. However, it is good to know how much of the data is missing. For example, say you have 100 samples, but 20 are NA. This affects the amount of replicates you have to work with. 
...
*Exercise* on your own play around with the data to explore how much missing data is in the dataset. 
...
It certainly seems that an extra 0 was added to the end. If you can confirm this, you can change the value to the correct one. Otherwise, you should make an NA. You can do this using na_if() in `dplyr`. In `na_if` you supply arguments for the name of the vector you are editing and the value that should be turned to NA
...
**Exercise**: How could you use mutate() and na_if() to deal with E. coli outlier. 
...
**Exercise** In the stream codes file ("raleigh_wq_stream_codes.csv", separate lat and long into two columns. You will need to set header = F in the `read.csv()` function
...
**Exercise** This date format comes in handy when plotting. Choose 1 variable in 1 stream and plot with Date on the y axis
...
1. Open the data in excel or google sheets. Make a list of things you would need to do to original data set to get into the tidy format. 
1. Play around with the data to explore how much missing data is in the dataset. 
1. How could you use mutate() and na_if() to deal with E. coli outlier. 
1. Use separate to make a column of lat and long in the stream codes. Use `write.csv()` to save the file so you can use later. 
1. Choose 1 variable and 1 stream and plot with Date on the y axis
1. Join stream precipitation data into water quality data set, using the dataset 'precip_stream_sites.csv'. This data set has the precipitation for each stream site for every day from 2008 - 2023. Choose one parameter (either E_coli_MPN, TSS_mg_L, Copper_mg_L) and run an analysis to see if the amount of rainfall impacts the value of the parameter. Include all code and a paragraph of your conclusions. 
